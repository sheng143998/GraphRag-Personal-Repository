...
class StudyAgentWorkflow:
    ...
    async def run(self, *, payload: AgentInvokeRequest, trace_builder: TraceBuilder) -> AgentWorkflowState:
        if is_support_request(payload):
            raise ValueError("Support requests must be routed through SupportSupervisorWorkflow.")
    ...
    def _generate_study_plan(self, state: AgentWorkflowState, trace_builder: TraceBuilder) -> None:
        ...
        trace_builder.set_attribute("study_plan", model_to_dict(state.study_plan))
        self._record_step(
            state,
            trace_builder,
    ...
    def _generate_review_cards(self, state: AgentWorkflowState, trace_builder: TraceBuilder) -> None:
        ...
        trace_builder.set_attribute("review_cards", models_to_dicts(cards))
        self._record_step(
            state,
            trace_builder,
