...
- 所有中文文件按 UTF-8 读取；Windows PowerShell 必须显式使用 `-Encoding UTF8`；禁止用裸 `Get-Content` 判断中文内容。
...
- 继续完成 Agent 编排框架化：AI 服务主依赖新增 `langgraph==0.1.5` 与 `langchain-core==0.2.8`，在保持 `FastAPI 0.95.2 + Pydantic 1.10.26` 不升级的前提下让 `SupportSupervisorWorkflow` 默认 `auto` 运行时可走真实 LangGraph `StateGraph.compile().ainvoke(...)`；新增 `AI_AGENT_SUPPORT_WORKFLOW_RUNTIME=auto|langgraph|local`，显式 `langgraph` 模式失败不静默回落，默认 `auto` 模式仅在依赖缺失或图构建前失败时写入可观测 fallback reason，图执行期失败直接抛出并记录 runtime error，避免复用脏 state 重跑 local。
- 新增 LangChain Core 节点/工具适配：售后 supervisor 节点通过 `RunnableLambda` 包装，代码/日志分析抽成 `analyze_support_log_patterns` 并提供 `build_support_log_pattern_tool()` StructuredTool；图 runtime 与 local runtime 共享同一批 `_node_*` 方法，避免两套编排逻辑漂移。
- 新增售后 Agent gate 状态契约：`SupportAgentState` 固定 `workflow_runtime`、`workflow_status`、`required_gates`、`completed_gates`、`skipped_gates`，trace 顶层同步 `final_status`；澄清早退是唯一允许跳过后续 gate 的终态，非澄清终态必须完成风险审查和评估审查。
- 修复售后安全闸门审查问题：旧 `StudyAgentWorkflow` 直接收到 support 请求时会拒绝执行，避免绕过 supervisor，同时移除旧学习 workflow 中不可达的售后分诊生成逻辑；评估审查移除 mojibake 中文兜底，乱码章节不再被当作有效“售后 / 风险 / 诊断”章节；评估失败会显式把 trace/status 标为 `needs_review`，并在最终回答和 `supportPlan.riskNotes` 中加入人工复核要求。
...
- 新增 RAGAS 评估体系第一阶段：AI 服务增加 RAGAS bridge，将现有 `RagEvaluateRequest` 映射为 `user_input`、`retrieved_contexts`、`response`、`reference`、`retrieved_context_ids` 和 `reference_context_ids`；RAGAS 依赖保持懒加载，当时用于避免 Pydantic v2 与旧 FastAPI/Pydantic v1 主服务冲突。
