# 2026-05-27 RAG 实验创建接口过程记录

## 背景

本次任务是新增 `POST /api/rag/experiments`，用于创建 RAG 实验配置或手动录入实验结果。
...
- 根因分析：这是上一轮已出现的本地 Maven 依赖 jar 占用或权限问题。当前编译和测试生命周期最终成功，说明没有阻塞业务代码验证。
- 解决方案：本轮不做破坏性清理，只记录该本地环境问题。
- 后续避免方式：如需彻底清理，应只处理该单个依赖目录，避免删除整个本地仓库。
- 是否补充自动化测试：不涉及业务逻辑，无需新增自动化测试。

## 已执行验证

- `mvn test`：通过，当前项目没有 Java 测试源码，因此主要验证编译、资源复制和 Spring Boot 构建链路。
- 未执行真实 HTTP 创建 smoke：本轮没有启动 PostgreSQL 和后端服务。
