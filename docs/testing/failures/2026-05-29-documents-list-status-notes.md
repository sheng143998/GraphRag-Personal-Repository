# 2026-05-29 文档列表状态接口过程记录

## 背景

本次任务是增强 `GET /api/documents`，让上一轮入库 Demo 后能在列表页看到文档状态和 chunk 数量。
...
- chunk 数量统计可能先采用简单逐文档查询，后续需要按性能再优化。
- 当前本地 pytest 依赖缺失问题仍存在，本轮重点验证 Java 和前端构建。
