# 2026-06-08 LLM 查询 转换 回退

## 目标

本计划记录 `llm-query-transform-fallback` 相关工作的实现意图、边界和验证方式。该工作服务于本地知识库 Agent / Advanced RAG 项目，要求保持前端、Spring Boot 与 FastAPI 的职责边界清晰。
...
- 根据 `llm-query-transform-fallback` 的主题更新对应服务、测试或文档。
