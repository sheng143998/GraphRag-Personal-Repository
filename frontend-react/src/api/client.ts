...
function resolveBaseUrl(): string {
  ...
  return typeof value === "string" && value.trim().length > 0 ? value.trim().replace(/\/$/, "") : DEFAULT_BASE_URL;
}
...
export async function apiRequest<T>(path: string, options: RequestOptions = {}): Promise<T> {
  ...
  if (!response.ok) {
    throw new ApiError(extractErrorMessage(payload, `请求失败，状态码 ${response.status}`), response.status, traceId);
  }
