...
这是一个用于练习本地知识库、Advanced RAG、Agent 编排和 RAG 评测闭环的三服务项目。项目采用 `React + Spring Boot + FastAPI + PostgreSQL/pgvector` 架构，重点不是只做一个问答页面，而是把文档入库、检索、生成、评估、实验对比和可观测性串成完整工程链路。
